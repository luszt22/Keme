...
export default function Topbar({ 
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center gap-6 h-14 px-6 border-b border-gray-200 bg-white">
      <a href="#" className="text-2xl font-black tracking-[-0.08em] flex items-center gap-0.5">
        <span>R</span>
      ...
      <nav className="hidden lg:flex items-center gap-16 ml-12">
        {['Discover', 'Marketplace', 'Create', 'Robux'].map((item) => (
          <a
            key={item}
            href="#"
            className="text-[15px] font-black text-[#232527] hover:text-[#0074BD] transition-all uppercase tracking-[0.08em] opacity-90 hover:opacity-100"
          >
            {item}
          </a>
        ))}
      ...
      <form 
        ...
        className="max-w-[440px] hidden lg:flex flex-1 items-center gap-2 bg-[#EBEDF0] border border-[#DEE1E5] px-3 py-1.5 rounded-md text-[#616A72] group focus-within:border-zinc-400 focus-within:bg-white transition-all relative ml-8"
      >
